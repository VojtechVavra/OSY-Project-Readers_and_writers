...
void *socketThread(void *arg)
{
    ...
    ReadLine readLine = {
        .fd = newSocket,
        .stored_buffer = {0},
        .emptyBuffer = emptyBufferFunc,
        .buffRead = buffReadFunc
    };

    int expected_message = FIRST_EXPECTED_MESSAGE;
    ...
    while (1)
    {
        readLine.emptyBuffer(&readLine);
        int l = 0;
        //printf(" [in loop]\n");

        // read data from client socket
        while ((l = readLine.buffRead(&readLine)) > 0)
        {
        ...
        int check_format = stringParser(readLine.buf, &msg);
        if (!check_format)
        {
        ...
        l = write(STDOUT_FILENO, readLine.buf, l);
        if (l < 0)
        {
        ...
        if (expected_message == FIRST_EXPECTED_MESSAGE)
        {
            ...
            if (msg.number == CI_PrichaziC && !strcmp(msg.text, CS_PrichaziC))
            {
                ...
                // 4. Muzes cist
                send_message(newSocket, ANSWER, AI_Cti, AS_Cti);
                // enum has global scope in c opposit to c++ where is need to write MessageType::ANSWER
            }
            ...
            else if (msg.number == CI_PrichaziS && !strcmp(msg.text, CS_PrichaziS))
            {
                ...
                // 12. Muzes psat
                send_message(newSocket, ANSWER, AI_Pis, AS_Pis);
            }
            else
            {
                continue;
        ...
        else if (expected_message == CI_Cist)
        {
            ...
            if (msg.number == CI_Cist)
            {
                ...
                send_message(newSocket, ANSWER, AI_Data, AS_Data);
            }
            else
            {
                //printf("\nespravna zprava!\n");
        ...
        else if (expected_message == CI_Psat)
        {
            ...
            if (msg.number == CI_Psat)
            {
                ...
                // 14. send hotovo
                send_message(newSocket, ANSWER, AI_Zapsano, AS_Zapsano);
            }
            else
            {
                continue;
        ...
        else if (expected_message == CI_KONEC_CTENAR || expected_message == CI_KONEC_SPISOVATEL)
        {
            ...
            {
                // 8. send naschledanou
                send_message(newSocket, ANSWER, AI_Nashledanou, AS_Nashledanou);
...
int main(int argc, char *argv[])
{
    ...
    *num_readers = 0;
    ...
    struct in_addr addr_any = {INADDR_ANY};
    ...
    if (bind(sock_listen, (const struct sockaddr *)&srv_addr, sizeof(srv_addr)) < 0)
    {
        ...
        while (1) // wait for new client
        {
            ...
            { // new client?
                ...
                *sock_client = -1;
                ...
                // new connection
                *sock_client = accept(sock_listen, (struct sockaddr *)&rsa, (socklen_t *)&rsa_size);
                if (*sock_client == -1)
                {
                ...
                uint32_t lsa = sizeof(srv_addr);
                // my IP
                getsockname(*sock_client, (struct sockaddr *)&srv_addr, &lsa);
                log_msg(LOG_INFO, "My IP: '%s'  port: %d",
                        inet_ntoa(srv_addr.sin_addr), ntohs(srv_addr.sin_port));
                // client IP
                getpeername(*sock_client, (struct sockaddr *)&srv_addr, &lsa);
